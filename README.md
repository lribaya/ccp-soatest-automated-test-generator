...
This Python script creates and configures a generated .tst based on a template .tst and a CSV file. It calls the SOAtest REST API to create the new test file, copy suites from the template, and update tools using the CSV data. An optional post-processing step can also clean up the generated .tst on disk.
...
2. **Create a New Test File**:
   - Sends a POST request to create the new .tst (default: `CCPPlan_generated.tst`).

3. **Set Execution Options**:
   - Configures the top-most test suite to run tests concurrently.
...
5. **Optional Post-Processing**:
   - If `--GENERATED_TST_PATH` is provided, the script updates the generated .tst on disk to:
     - Keep a single `suite.environmentConfig.environments` item (prefers the one with `active: true`).
     - Remove top-level `suite.authentications` blocks.
     - Promote one nested `suite.tests.authentications` OAuth2 block to top-level and remove other nested OAuth2 blocks.
...
- **Generated .tst Post-Processing**:
  - Provide `--GENERATED_TST_PATH` with the full path to the generated .tst file.
...
2. Run the script (PowerShell example):
   ...
   ```
3. To enable post-processing on the generated .tst:
   ```powershell
   ...
   ```
4. To log all API calls, add `--DEBUG`:
   ```powershell
...
This script is tailored for a specific .tst template and may require modifications to work with other templates. Use it as a reference or starting point for automating test creation for other use cases.
